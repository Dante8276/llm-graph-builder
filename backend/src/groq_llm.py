...
def get_graph_from_groq(model_id, chunkId_chunkDoc_list: List, allowedNodes, allowedRelationship):
    futures = []
    graph_document_list = []
    combined_chunk_document_list = get_combined_chunks(chunkId_chunkDoc_list)
    llm_1 = ChatGroq(temperature=0, model_name=model_id, groq_api_key=os.environ.get('GROQ_API_KEY_1'))
    llm_2 = ChatGroq(temperature=0, model_name=model_id, groq_api_key=os.environ.get('GROQ_API_KEY_2'))
    llm_transformer_1 = LLMGraphTransformer(llm=llm_1, allowed_nodes=allowedNodes, allowed_relationships=allowedRelationship)
    llm_transformer_2 = LLMGraphTransformer(llm=llm_2, allowed_nodes=allowedNodes, allowed_relationships=allowedRelationship)
    max_workers = 30
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for i, chunk in enumerate(combined_chunk_document_list):
            if i < len(combined_chunk_document_list) // 2:
                futures.append(executor.submit(llm_transformer_1.convert_to_graph_documents, [chunk]))
            else:
                futures.append(executor.submit(llm_transformer_2.convert_to_graph_documents, [chunk]))
